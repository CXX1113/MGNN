...
class Linear(nn.Module):
    ...
    def reset_parameters(param, initializer):
        if initializer == 'Xavier_Uniform':
            ...
            # init.kaiming_uniform_(param, a=math.sqrt(5))  # math.sqrt(5)
...
class MotifConv(nn.Module):
    def __init__(self,
        ...
        self.conv = GraphConv(in_channels, out_channels, norm='none', weight=False, bias=False)
    ...
    def forward(self, g, inputs):
        ...
        h = self.conv(g, inputs, weight=self.weight, edge_weight=g.edata['edge_weight_norm'])
